...
+ (instancetype)presentForTorrent:(Torrent*)torrent
{
    ...
    BtpkUpdatePanelController* controller = [[BtpkUpdatePanelController alloc]
        initWithWindowNibName:@"BtpkUpdatePanelController"];
...
- (void)windowDidLoad
{
    ...
    self.fKeyField.placeholderString = NSLocalizedString(
        @"Paste private key (PEM format)",
        "btpk update panel -> key field placeholder");
...
{
    NSString* pemString = [self.fKeyField.stringValue
    ...
    if (!keyOpt)
    {
        self.fKeyStatusField.stringValue = NSLocalizedString(
            @"Invalid key \u2014 paste the PEM block from your password manager.",
    ...
    {
        self.fKeyStatusField.stringValue = NSLocalizedString(
            @"This key does not match the torrent\u2019s public key.",
    ...
    self.fKeyStatusField.stringValue = NSLocalizedString(
        @"Scanning files and publishing\u2026",
        "btpk update panel -> publishing status");
    self.fKeyStatusField.textColor = NSColor.secondaryLabelColor;
    self.fPublishButton.enabled = NO;
    self.fCancelButton.enabled = NO;

    [self.fTorrent publishBtpkUpdateWithKeyData:keyData
                              completionHandler:^(NSString* _Nullable newMagnetLink, NSError* _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            else
            {
                if (newMagnetLink)
                {
                    ...
                    [NSPasteboard.generalPasteboard setString:newMagnetLink
                                                      forType:NSPasteboardTypeString];
                }
                NSAlert* alert = [[NSAlert alloc] init];
                alert.messageText = NSLocalizedString(
                    @"Update published.",
                    "btpk update -> success title");
                alert.informativeText = NSLocalizedString(
                    @"The magnet link has been copied to the clipboard.",
                    "btpk update -> success body");
...
{
    NSString* trimmed = [self.fKeyField.stringValue
        stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
